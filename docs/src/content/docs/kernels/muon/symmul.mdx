...
```python
newton_schulz_symmul(G, coeffs=_DSV4_COEFFS, ns_dtype=torch.float16, eps=1e-7, force_eager=False)
```
...
| `coeffs` | `_DSV4_COEFFS` | Per-iteration `(a, b, c)` tuples; the count **is** the iteration count. |
