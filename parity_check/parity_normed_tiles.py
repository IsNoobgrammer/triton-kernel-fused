"""Regression check: the RMS-normed act codes (2, 8) must take the Triton grouped GEMMs.

The fused GEMM+GLU epilogue genuinely cannot serve the normed codes (it holds a BN-wide slice of a row,
...
BiBo shape rather than returning None, and (c) numerics are unchanged vs moe_eager. Run on the box:
    python parity_check/parity_normed_tiles.py
"""
import importlib
...
CODES = {0: "silu", 2: "normsilu",
         # MIXED stacks: `uniform` in moe_per_expert means "no special experts, no act_params",
         # NOT "one code", so 0+2 still takes the batched grouped path. It loses only the fused
