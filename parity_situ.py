...
def main():
    ...
    # code 6 = radial NormSiLU (sqrt(r)*SiLU(g/r)); no act_params (raw). fp32 tight + bf16 loose, both paths.
    ok &= check("all-radial fp32", [6] * 6, raw=True)
    ok &= check("mixed-radial fp32", [0, 2, 6, 0, 2, 6], raw=True)
    ok &= check("radial+situ fp32", [2, 5, 6, 0], raw=False)                 # 6 ignores ap, 5 uses it
    ok &= check("radial pure-bf16", [6] * 6, raw=True, dtype=torch.bfloat16, tol=3e-2)
