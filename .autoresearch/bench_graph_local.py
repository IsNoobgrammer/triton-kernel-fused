...
if __name__ == "__main__":
    ...
    t_grf, m_grf, g = measure(lambda ps: FusedMuon(ps, lr=.02, weight_decay=.1, use_symmul=False, use_graph=True),  shapes)
    print(f"  symmul   eager  (use_graph=False):  {t_sym:8.3f} ms   {m_sym:7.0f} MB   <- amalgamated default")
    print(f"  cuBLAS   eager  (use_graph=False):  {t_cub:8.3f} ms   {m_cub:7.0f} MB")
    print(f"  cuBLAS   graph  (use_graph=True):   {t_grf:8.3f} ms   {m_grf:7.0f} MB   graph_captured={g}")
    print(f"  -> symmul vs cuBLAS-eager: {t_cub/t_sym:.2f}x | graph vs cuBLAS-eager: {t_cub/t_grf:.2f}x"
          f" | symmul(default) vs graph: {t_grf/t_sym:.2f}x")
    print("DONE")
