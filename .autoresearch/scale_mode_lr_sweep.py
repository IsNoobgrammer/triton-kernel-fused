...
def parse_arm(arm):
    mode, _, wd = arm.partition("@")
    return mode, (float(wd) if wd else WD_DEFAULT)
...
def run(arm, seed, lr, S, steps, h, i, e, blocks):
    mode, wd = parse_arm(arm)
    ...
    return dict(mode=arm, seed=seed, lr=lr, train=trl, id=idl, id_acc=ida, ood=oodl, ood_acc=ooda)
...
def main():
    ...
    ap.add_argument("--arms", default=",".join(MODES))
    ...
    if a.phase == "lr":
        ...
        json.dump(best, open(os.path.join(HERE, f"scale_mode_best_lr{a.tag}.json"), "w"))
        print("\n-> wrote scale_mode_best_lr.json; now run --phase seeds")
    else:
        best = json.load(open(os.path.join(HERE, f"scale_mode_best_lr{a.tag}.json")))
        ...
        for ref in (ARMS[0],):
            ...
            for k in ("id_acc", "ood_acc", "id"):
                for mode in ARMS:
                    if mode == ref or ref not in got[k]:
                        continue
    ...
    json.dump(R, open(os.path.join(HERE, f"scale_mode_lr_{a.phase}{a.tag}.json"), "w"))
