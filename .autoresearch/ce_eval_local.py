...
# #1: int8 saved-logits (skip recompute GEMM in backward) — grad_rel is the gate
for tag, bud in [("ours_int8_384MB", 384 * MB), ("ours_int8_128MB", 128 * MB)]:
    ms, pk, rel = run(tag, lambda h, w, l, _b=bud: fused_linear_cross_entropy(h, w, l, bwd_logits_budget=_b, bwd_mode="int8"))
    print(f"      -> latency {ms/base_ms:.2f}x baseline | peak {base_pk/max(pk,1):.2f}x less than baseline")
