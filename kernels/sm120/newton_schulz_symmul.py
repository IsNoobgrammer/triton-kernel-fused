...
class AmalgamatedMuon(_FusedMuon120):
    """FusedMuon (foreach launch-collapse + batched same-shape state + Blackwell mem knee) with the
    Newton-Schulz GEMMs done by the symmetric `symmul` kernel. This is the actual amalgamation: our
    optimizer-level levers (the ones that beat compiled ~2.3x on a whole model) COMPOSE with the
    symmetric FLOP cut. The champion FusedMuon is untouched; this only swaps the NS function.

    The eager `step` below is FusedMuon's eager step verbatim except `newton_schulz(...)` ->
    `newton_schulz_symmul(...)` (which itself gates to the champion NS below the gram knee).
    """
